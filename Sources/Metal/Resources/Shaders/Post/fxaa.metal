...
fragment float4 fragment_post_fxaa(
    UnlitRasterizerData data       [[stage_in]],
    texture2d<float> input         [[texture(0)]],
    constant FXAAOptions& options  [[buffer(4)]]
) {
    constexpr sampler s(
        address::repeat,
        mag_filter::linear,
    ...
    float3 rgb_north_west = input.sample(s, data.uv + float2(-1.0, -1.0) * texel_size).rgb;
    float3 rgb_north_east = input.sample(s, data.uv + float2(1.0, -1.0) * texel_size).rgb;
    float3 rgb_south_west = input.sample(s, data.uv + float2(-1.0, 1.0) * texel_size).rgb;
    float3 rgb_south_east = input.sample(s, data.uv + float2(1.0, 1.0) * texel_size).rgb;
    float3 rgb_current = input.sample(s, data.uv).rgb;
    ...
    float3 rgb_near = (1.0/2.0) * (
        input.sample(s, data.uv + direction * (1.0/3.0 - 0.5)).rgb +
        input.sample(s, data.uv + direction * (2.0/3.0 - 0.5)).rgb
    );
    
    float3 rgb_far = rgb_near * (1.0/2.0) + (1.0/4.0) * (
        input.sample(s, data.uv + direction * (0.0/3.0 - 0.5)).rgb +
        input.sample(s, data.uv + direction * (3.0/3.0 - 0.5)).rgb
    );
    
    float luma_far = dot(rgb_far, luma);
    
    return float4((luma_far < luma_min || luma_far > luma_max) ? rgb_near : rgb_far, 1.0);
}
